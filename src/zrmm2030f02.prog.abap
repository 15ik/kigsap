...
**  DATA : LV_TEXT(255) TYPE C.
  ...
  CONSTANTS : LC_LINE1(20) TYPE C VALUE 'LTP 시나리오 : ',
              LC_LINE2(20) TYPE C VALUE '현재고 포함 기초 재고 계산 : ',
...
**    READ TABLE S_ZMONTH INDEX 1.
**    IF S_ZMONTH-HIGH IS NOT INITIAL.
**      DATA(LV_ZMONTH_TXT) = |{ S_ZMONTH-LOW } ~ | && |{ S_ZMONTH-HIGH }|.
**    ELSE.
**      LV_ZMONTH_TXT = S_ZMONTH-LOW.
**    ENDIF.
**  ENDIF.
**
**  IF P_WERKS IS NOT INITIAL.
**    SELECT SINGLE FROM T001W FIELDS NAME1 WHERE WERKS = @P_WERKS INTO @DATA(LV_WERKS_TXT).
**  ENDIF.

  SELECT SINGLE FROM PLSC FIELDS PLSCT WHERE PLSCN = @P_PLSCN INTO @DATA(LV_PLSCT).
  DATA(LV_LINE1_TXT) = |{ P_PLSCN } | && LV_PLSCT.

  DATA(LV_LINE2_TXT) = COND CHAR4( WHEN P_CHK1 IS INITIAL THEN TEXT-004
                                   ELSE                   TEXT-O05 ).
  ...
  LOOP AT CT_FCAT ASSIGNING FIELD-SYMBOL(<LS_FCAT>).
    ...
    CASE <LS_FCAT>-FIELDNAME.
      WHEN 'MATNR'.
        ...
        <LS_FCAT>-CONVEXIT = TEXT-O07. "ALPHA
      WHEN 'MAKTX'.
        _L_SET_FCAT: 2   ''  ''   '내역'    '' ''  '' '' ''.
        <LS_FCAT>-OUTPUTLEN = 40.
      WHEN 'GSMNG'.
        _L_SET_FCAT: 3   ''  ''   '계획량'    '' ''  '' 'MARA_MEINS' ''.
        <LS_FCAT>-OUTPUTLEN = 10. <LS_FCAT>-DO_SUM = 'X'.
      WHEN 'MARA_MEINS'.
        _L_SET_FCAT: 4   '' ''  '단위'     'C'  ''  '' '' ''.
        <LS_FCAT>-OUTPUTLEN = 4.
      WHEN 'REQ_QTY'.
        _L_SET_FCAT: 5   ''  ''   '소요량'    '' ''  '' 'MEINS' ''.
        <LS_FCAT>-OUTPUTLEN = 10. <LS_FCAT>-DO_SUM = 'X'.
      WHEN 'MEINS'.
        _L_SET_FCAT: 6   '' ''  '소요량 단위'     'C'  ''  '' '' ''.
        <LS_FCAT>-OUTPUTLEN = 6.
