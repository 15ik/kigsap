...
FORM GET_CHECK_DATA.
  ...
  CLEAR: GT_USER_REQ, GT_USER_ORD, GT_T001W, GT_T001L, GT_ORG,
...
*U1> Tier2 를 위해 Text Symbol 로 변환 - START
*    LV_FNAME        TYPE STRING VALUE 'PR 일괄 생성 Error List',
    LV_FNAME        TYPE STRING,
*U1> Tier2 를 위해 Text Symbol 로 변환 - END
...
*-----------------------------
  SELECT SINGLE RELID,
    ...
    FROM WWWDATA
   WHERE OBJID = @GC_OBJID.

  IF LS_WWWDATA_ITEM IS INITIAL.
    "SAP 웹저장소(SMW0)에 오브젝트 &이 등록되지 않았습니다.
    MESSAGE S002 WITH TEXT-M98 GC_OBJID TEXT-M99 DISPLAY LIKE 'E'.
    EXIT.
  ENDIF.
...
*-----------------------------
  TRY.
      ...
      CONCATENATE TEXT-T02 LC_EXTENTION INTO LV_FNAME. "PR 일괄 생성 Error List.XLSX
...
* 엑셀 라인추가
  IF GT_ERROR IS NOT INITIAL.
    ...
    PERFORM LINE_INSERT USING 4 LV_CNT.
  ENDIF.
...
FORM MAKE_ERROR_DATA.
  ...
  LOOP AT GT_DISP INTO DATA(LS_DISP).
    ...
    "오류 Data만 다운로드
    IF LS_DISP-STATUS = ICON_RED_LIGHT.

      "단가 Conversion
      PERFORM CURRENCY_CONV_TO_EXTERNAL USING    LS_DISP-WAERS
                                        CHANGING LS_DISP-PREIS.

      MOVE-CORRESPONDING LS_DISP TO LS_ERROR.
      ...
      CLEAR: LS_ERROR, LS_LINE, LS_DISP.
    ENDIF.
